<?php
	...
	$query = "SELECT a.id, a.imie, a.nazwisko, a.dzial , c.id, c.nazwa FROM  ludzie a left join ludz_zes b on b.id_ludz = a.id left join zestawy c on c.id = b.id_zest ORDER BY a.nazwisko ASC, a.imie ASC";
	$result = mysql_query($query);
	if (!$result) die ("Database access failed: " . mysql_error());
	$rows = mysql_num_rows($result);
